...
def main():
    """
    Main function to find the single best-fit constant sigma for a dataset, given a fixed r.
    """
    # --- 1. การตั้งค่า Directory และ Logging ---
    base_output_dir = os.path.join("btcusdt_options_call_V2", "real_find_best_fit_sigma")
    run_number = 1
    while True:
    ...
    # --- 4. กำหนดค่า r และสร้าง sigma ให้เป็นพารามิเตอร์ที่เรียนรู้ได้ ---
    r_fixed = torch.tensor(p['fixed_r'], device=DEVICE)
    sigma_param = nn.Parameter(torch.tensor(0.8, device=DEVICE)) # ให้ค่าเริ่มต้นที่สมเหตุสมผล

    # สร้าง Optimizer ที่จะปรับค่าแค่ sigma
    optimizer = torch.optim.Adam([sigma_param], lr=p['learning_rate'])
    loss_fn = nn.MSELoss()

    logging.info(f"Initial sigma: {sigma_param.item():.4f}")
    ...
    for epoch in range(p['epochs']):
        ...
        V_predicted = eur_call_analytical_price(S_data, t_data, K_data, r_fixed, torch.abs(sigma_param), T_data)
        ...
        if (epoch + 1) % 1000 == 0:
            logging.info(f'Epoch [{epoch+1}/{p["epochs"]}], Loss: {loss.item():.8f}, sigma: {torch.abs(sigma_param).item():.4f}')

    logging.info("Optimization finished.")
    final_sigma = torch.abs(sigma_param).item()
    logging.info(f"--- Best-fit Constant Parameters ---")
    logging.info(f"Fixed r: {p['fixed_r']:.6f}")
    logging.info(f"Final Best-fit sigma: {final_sigma:.6f}")
    logging.info("------------------------------------")

    # --- 6. พล็อตกราฟแสดงผล ---
    with torch.no_grad():
        V_final_predicted = eur_call_analytical_price(S_data, t_data, K_data, r_fixed, torch.abs(sigma_param), T_data)
    ...
    plt.title('Loss Curve for Best-fit Sigma')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.yscale('log')
    plt.grid(True)
